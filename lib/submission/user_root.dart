...
class UserRoot extends StatefulWidget {
  ...
  const UserRoot({super.key, this.userId, this.isEditing = false, this.userDetails});
...
class _UserRootState extends State<UserRoot> {
  ...
  confirmPayment() async {
    ...
    if (enteredAmount != totalCost) {
      ...
      if (widget.isEditing) {
        await DatabaseMethods().updateUserDetails(id, userInfoMap, collection: 'Root detail form');
      } else {
        await DatabaseMethods().addUserDetails(userInfoMap, id, collection: 'Root detail form');
      }

      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => RootConfirmationScreen(userId: id, totalCost: totalCost),
        ),
      );
    }
  }
...
class RootConfirmationScreen extends StatelessWidget {
  ...
  const RootConfirmationScreen({Key? key, required this.userId, required this.totalCost}) : super(key: key);
